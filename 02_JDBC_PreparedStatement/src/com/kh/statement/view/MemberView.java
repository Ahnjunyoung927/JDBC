...
public class MemberView {
	...
	private MemberController mc = new MemberController(); // 합성 : 상속하지 않고 필드로 두고 사용
	 ...
	 * 
	 * 컬럼에 INSERT할 값을들 모두 입력 받은 후 입력받은 값을 컨트롤러로 전달
	 */
	private void save() {
	...
	private void findAll() {
		...
		// 컨트롤러에게 회원들의 데이터 값 달라고 요청
		List<Member> members = mc.findAll();
		
		// 뷰에서 2절
		System.out.println("\n 조회된 총회원수는 " + members.size() + "명 입니다.");
		if(members.isEmpty()) {
	...
	private void findById() {
		...
		// 2. 조회결과가 존재할 경우        == VO객체(Member객체)의 주소값
	...
	private void findByKeyword() {
		...
		if(members.isEmpty()) {
			...
			for(int i = 0; i < members.size(); i++) {
				System.out.println((i + 1) + "번째 조회결과!");
